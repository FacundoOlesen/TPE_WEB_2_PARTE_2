...
class ProductApiController
{
    ...
    {
        if (($sortby == 'id') || ($sortby == 'nombre') || ($sortby == 'precio') || ($sortby == 'talle') || ($sortby == 'descripcion') || 
        ($sortby == 'id_categoria_fk') || ($sortby == 'imagen')  && ($order == "asc") || ($order == "desc")) {
            try {
    ...
    {
        if (isset($filterby) && isset($value) && ($filterby == 'id') || ($filterby == 'nombre') || ($filterby == 'precio') || ($filterby == 'talle') 
        || ($filterby == 'descripcion') || ($filterby == 'id_categoria_fk') || ($filterby == 'imagen') && in_array($value, $this->model->getAllProducts())) {
            try {
    ...
    private function getProductsFilteredAndOrdered($filterby, $value, $sortby, $order) {
        if (
            isset($filterby) && isset($value) && ($filterby == 'id') || ($filterby == 'nombre') ||
            ($filterby == 'precio') || ($filterby == 'talle') || ($filterby == 'descripcion') || ($filterby == 'id_categoria_fk') ||
            ($filterby == 'imagen') && in_array($value, $this->model->getAllProducts()) && ($sortby == 'id') || ($sortby == 'nombre')
            || ($sortby == 'precio') || ($sortby == 'talle') || ($sortby == 'descripcion') || ($sortby == 'id_categoria_fk') ||
            ($sortby == 'imagen')  && ($order == "asc") || ($order == "desc")
        ) {
            try {
    ...
    public function insertProducts($params = null)
    {
        ...
        if (empty($products->nombre) || empty($products->precio) || empty($products->talle) || empty($products->descripcion) || 
        empty($products->id_categoria_fk) || empty($products->imagen)) {
            $this->view->response("Error. Falto completar algún campo.", 400);
            die();
        }

        try {
            $id = $this->model->insertProduct($products->nombre, $products->precio, $products->talle, $products->descripcion,  $products->id_categoria_fk,  
            $products->imagen);
            $product = $this->model->getProductsById($id);
    ...
    {
        if (!$this->authHelper->isLoggedIn()) {
            $this->view->response("Error! Para modificar un producto debes estar logueado.", 401);
            return;
        }
        $id = $params[':ID'];
        $product = $this->getData();

        if (empty($product->nombre) || empty($product->precio) || empty($product->talle) || empty($product->descripcion) || empty($product->id_categoria_fk) 
        || empty($product->id)) {
            $this->view->response("Error! Dejaste campos vacios.", 400);
            die();
        } else {
        ...
        try {
            if ($products) {
                $this->model->modifyProduct($product->nombre, $product->precio, $product->talle, $product->descripcion, $product->id_categoria_fk, $product->id);
                $this->view->response("Editaste el producto con el id $id!", 200);
            } else {
                $this->view->response("El producto con el id $id no existe", 404);
            }
        } catch (Exception $e) {
